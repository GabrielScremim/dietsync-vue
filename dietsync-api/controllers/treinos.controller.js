...
exports.createTreino = async (req, res) => {
  try {
    const { data, tipo, exercicios, repeticoes, series, objetivo, duracao, frequencia, nome_treino, dia_treino, fk_id_usuario_treino = 40 } = req.body;

    const sql = `INSERT INTO treino (data, tipo, exercicios, repeticoes, series, objetivo, duracao, frequencia, nome_treino, dia_treino, fk_id_usuario_treino) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    const [result] = await db.query(sql, [data, tipo, exercicios, repeticoes, series, objetivo, duracao, frequencia, nome_treino, dia_treino, fk_id_usuario_treino]);

    res.json({ message: "Treino cadastrado com sucesso!", treinoId: result.insertId });
  } catch (err) {
...
exports.updateTreino = async (req, res) => {
  try {
    const { id, id_treino } = req.params;
    const { data, tipo, nome_treino, exercicios, repeticoes, series, objetivo, duracao, frequencia } = req.body;

    const [result] = await db.query(
      `UPDATE treino SET data = ?, tipo = ?, nome_treino = ?, exercicios = ?, repeticoes = ?, series = ?, objetivo = ?, duracao = ?, frequencia = ? 
       WHERE id = ? AND fk_id_usuario_treino = ?`,
      [data, tipo, nome_treino, exercicios, repeticoes, series, objetivo, duracao, frequencia, id_treino, id]
    );

    if (result.affectedRows === 0) return res.status(404).json({ message: "Treino não encontrado" });
